...
void logmsg(const char *str, va_list argp) {
  char *buf = calloc(strlen(str) + 2 + 1, sizeof(char));
  sprintf(buf, "%s\n", str);
  vfprintf(chat.debug, buf, argp);
  free(buf);
}
...
void refresh_input(void) {
  ...
  wclear(chat.chatbox);
  box(chat.chatbox, '|', '-');
  mvwprintw(chat.chatbox, 0, 3, " Type message... ");
